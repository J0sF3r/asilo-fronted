...
export const generarPDFCobros = (datos, familiar, fechaInicio, fechaFin) => {
    ...
    // Encabezado (ajustado para el logo)
    doc.setFontSize(18);
    ...
    doc.text(`Fecha de emisión: ${new Date().toLocaleDateString('es-GT', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
        hour: '2-digit',
    ...
    doc.text(`Q${datos.totalCargos}`, 80, finalY + 16);

    doc.text('Total Descuentos:', 20, finalY + 23);
    doc.setTextColor(46, 125, 50);
    doc.text(`-Q${datos.totalDescuentos}`, 80, finalY + 23);

    doc.setTextColor(0, 0, 0);
    doc.text('Total Pagado:', 20, finalY + 30);
    doc.text(`Q${datos.totalPagado}`, 80, finalY + 30);

    doc.setFont(undefined, 'bold');
    doc.setFontSize(11);
    doc.text('Balance Pendiente:', 110, finalY + 23);
    doc.setTextColor(211, 47, 47);
    doc.text(`Q${datos.balancePendiente}`, 170, finalY + 23);

    // Footer
    doc.setFontSize(8);
    doc.setTextColor(128, 128, 128);
    doc.setFont(undefined, 'normal');
    doc.text('Este documento es un reporte generado electrónicamente', 105, 285, { align: 'center' });
